...
/** 
 * Cette classe statique implémente l'algorithme de A* de recherche de plus court chemin.
 * Vous trouverez une très bonne description de l'algorithme sur la page anglophone associée de Wikipédia :
...
public class AStar {
	...
	/**
	 * Calcule l'heuristique par rapport à la destination utilisée dans l'algorithme de A*.
	 * Ici on utilise la distance de Manhattan en guise d'heuristique.
	...
	/**
	 * Cette fonction retourne le chemin que le robot doit parcourir en remontant la liste retournée par A*.
	 * @param path			le chemin retourné par la fonction trouveChemin
	 * @param destination	la destination finale du robot, qui est aussi la dernière case visité par l'algorithme de A*
	 * @return				la suite des cases que le chemin doit parcourir de la position deb à la position fin
	 */
	...
	/**
	 * Cette fonction implémente l'interface Comparator<Case> dans le but d'utiliser une PriorityQueue sur les cases.
	 * Cette PriorityQueue trie les cases en fonction de leur cout f = g + h (:= la distance + l'heuristique).
	 */
